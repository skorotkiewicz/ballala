...
export default class UnstoppableChat {
  ...
  async join(username: string, password: string) {
    ...
    return new Promise((resolve) => {
      gun.on("auth", (res) => {
        ...
        gun.user().get("epub").put(gun.user().is.epub);
        // gun.user().get("alias").put(gun.user().is.alias);
      ...
      gun.user().auth(username, password, (res) => {
        resolve(res);
      });
    });
  }
  ...
  async createUser(username: string, password: string) {
    ...
    return new Promise<void>((resolve) => {
      gun.user().create(username, password, (res) => {
        resolve(res);
      });
      // resolve();
    });
  ...
  async addContact(username: string, pubKey: string) {
    const gun = this.gun;
    const publicName = this.publicName;
  ...
  async acceptContactInvite(username: string, pubKey: string) {
    const gun = this.gun;
    const publicName = this.publicName;
  ...
  async loadMessagesOfContact(pubKey: string) {
    const gun = this.gun;
    const publicName = this.publicName;
                      ...
                      Object.keys(peers).forEach((pubKey) => {
                        ...
                        gun
                          ...
                          .once((peerData) => {
                            ...
                            loadedChannelsList[loadedChannelIndex].peers =
                              loadedPeers;
                            emitter.emit("channels", loadedChannelsList);
                          });
                      });
                      gun
                        ...
                        .on((newMsgs) => {
                          ...
                          if (loadedChannelsList[loadedChannelIndex]) {
                            loadedChannelsList[loadedChannelIndex].notifCount =
                              newCount;
                          }
                          emitter.emit("channels", loadedChannelsList);
                        });
  ...
  async acceptChannelInvite(
    ...
    if (!channel.peers[gun.user().is.pub]) {
      ...
      channel.peers[gun.user().is.pub] = { alias: gun.user().alias };
    }
  ...
  async createAnnouncement(
    ...
    if (isPrivate) {
      ...
      if (!hash) return;
      gun
        .get("#public/announcements/3")
        .get(hash)
        .put(JSON.stringify(announcement));
  ...
  loadPublicAnnouncements() {
    ...
    return {
      on: (cb: (param: Events["publicAnnouncements"]) => void) => {
        ...
        gun.get("#public/announcements/3").on((hashMap: any) => {
          Object.keys(hashMap).forEach((hash: string) => {
            if (hash === "_") {
              return;
            }
            gun
              ...
              .once((announcementStr: string) => {
                ...
                if (
                  !announcement ||
                  !announcement.key ||
                  loadedAnnouncements[announcement.key]
                ) {
                  return;
                }
  ...
  async joinPublicAnnouncement(publicAnnouncement: Channel) {
    ...
    gun
      ...
      .get("announcement")
      .get(publicAnnouncement.key)
      .put(publicAnnouncement);
    gun
      .user(publicAnnouncement.owner)
      .get("announcement")
      .get(publicAnnouncement.key)
      .get("admins")
      .once((admins: any) => {
    ...
    if (!hash) return;
    gun
      .get(`#public/announcements/3/${publicAnnouncement.key}/peers`)
      .get(hash)
      .put(JSON.stringify(peerData));
  ...
  async acceptAnnouncementInvite(
    ...
    if (!announcement.peers[gun.user().is.pub]) {
      ...
      announcement.peers[gun.user().is.pub] = { alias: gun.user().alias };
    }
        ...
        if (announcement.rssLink) {
          fetch("https://cors-anywhere.herokuapp.com/" + announcement.rssLink)
            ...
            .then((data) => {
              ...
              const rssTitle =
                data.querySelector("title")!.textContent || announcement.name;
